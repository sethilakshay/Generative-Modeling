...
class UpSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        ##################################################################
        x = torch.repeat_interleave(x, dim=1, repeats=self.upscale_factor*self.upscale_factor)
...
class DownSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        channels = channels//int(self.downscale_ratio**2)
        x = x.view(batch_size, channels, -1, height, width)
        x = x.permute(2, 0, 1, 3, 4).squeeze(0)
        x = self.conv(x.mean(dim = 0))
...
class ResBlockUp(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.upsample_residual = UpSampleConv2D(input_channels=input_channels, n_filters=n_filters, kernel_size=1, padding=0)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class ResBlockDown(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.downsample_residual = DownSampleConv2D(input_channels=input_channels, n_filters=n_filters, kernel_size=1, padding=0)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Generator(torch.jit.ScriptModule):
    ...
    def __init__(self, starting_image_size=4):
        ...
        self.layers = nn.Sequential(
            ResBlockUp(input_channels=128),         # Layer 0 (ResBlockUp)
            ResBlockUp(input_channels=128),         # Layer 1 (ResBlockUp)
            ResBlockUp(input_channels=128),         # Layer 2 (ResBlockUp)
            nn.BatchNorm2d(128, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True),     # Layer 3
            nn.ReLU(),                                                                               # Layer 4
            nn.Conv2d(128, 3, kernel_size=3, stride=1, padding=1),                                   # Layer 5
            nn.Tanh()                                                                                # Layer 6
        )
        ##################################################################
        #                          END OF YOUR CODE                      #
    ...
    def forward(self, n_samples: int = 1024):
        ...
        # network.
        ##################################################################
        z = torch.normal(mean=0., std=1., size=(n_samples, 128), dtype=torch.float16, device='cuda')
        z = self.dense(z)
        z = z.reshape(-1, 128, self.starting_image_size, self.starting_image_size)
        z = self.layers(z)
...
class Discriminator(torch.jit.ScriptModule):
    ...
    def __init__(self):
        ...
        ##################################################################
        self.layers = nn.Sequential(
            ResBlockDown(input_channels=3),      # Layer 0 (ResBlockDown)
            ResBlockDown(input_channels=128),    # Layer 1 (ResBlockDown)
            ResBlock(input_channels=128),        # Layer 2 (ResBlock)
            ResBlock(input_channels=128),        # Layer 3 (ResBlock)
            nn.ReLU()                            # Layer 4   
        )
    ...
    def forward(self, x):
        ...
        x = self.layers(x)
        x = x.sum(dim=(2, 3)).reshape(batch_size, -1)
        x = self.dense(x)
