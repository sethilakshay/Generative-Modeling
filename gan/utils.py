...
def interpolate_latent_space(gen, path):
    ...
    ##################################################################
    samples = torch.zeros(size=(100, 128), device='cuda' if torch.cuda.is_available() else 'cpu')
    interpolation = torch.linspace(-1, 1, 10, device='cuda' if torch.cuda.is_available() else 'cpu')
    x, y = torch.meshgrid(interpolation, interpolation)

    samples[:, 0] = x.reshape(-1,)
    samples[:, 1] = y.reshape(-1,)

    generations = gen.forward_given_samples(samples)
    generations_grid = torchvision.utils.make_grid(generations, nrow=10, normalize=True)
    torchvision.utils.save_image(generations_grid, path)
