...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        self.alphas_cumprod_prev = torch.cat((torch.tensor([1], device = self.alphas_cumprod.device), self.alphas_cumprod[:-1]))
        ...
        self.x_0_pred_coef_2 = torch.sqrt((1/self.alphas_cumprod - 1))
    ...
    def model_predictions(self, x_t, t):
        ...
        x_0 = coef_1 * x_t - coef_2 * pred_noise
    ...
    def predict_denoised_at_prev_timestep(self, x, t: int):
        ...
        # z = torch.randn(x.shape, device='cuda' if torch.cuda.is_available() else 'cpu') if t[0] > 0 else 0 
    ...
    def ddim_step(self, batch, device, tau_i, tau_isub1, img, model_predictions, alphas_cumprod, eta):
        ...
        tau_i_tensor = torch.full((batch,), tau_i, device=device, dtype=torch.long)
        tau_isub1_tensor = torch.full((batch,), tau_isub1, device=device, dtype=torch.long)

        pred_noise, x_0 = model_predictions(img, tau_i_tensor)

        # To prevent Runtime CUDA error when tau_isub1 is negative
        if tau_isub1 < 0:
            return img, x_0

        # Step 2: Extract \alpha_{\tau_{i - 1}} and \alpha_{\tau_{i}}
        alpha_tau_i = extract(alphas_cumprod, tau_i_tensor, x_0.shape)
        alpha_tau_isub1 = extract(alphas_cumprod, tau_isub1_tensor, x_0.shape)

        # Step 3: Compute \sigma_{\tau_{i}}
        sigma_tau_i = torch.sqrt(eta * (1 - alpha_tau_isub1) /(1 - alpha_tau_i))

        # Step 4: Compute the coefficient of \epsilon_{\tau_{i}}
        epsilon_tau_i = torch.sqrt(1 - alpha_tau_isub1 - sigma_tau_i**2)

        # Step 5: Sample from q(x_{\tau_{i - 1}} | x_{\tau_t}, x_0)
        # HINT: Use the reparameterization trick
        z = torch.randn(img.shape, device=device) if tau_i > 0 else 0 
        img = (torch.sqrt(alpha_tau_isub1) * x_0) + (epsilon_tau_i * pred_noise) + (sigma_tau_i * z)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
