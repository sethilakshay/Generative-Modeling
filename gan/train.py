...
def train_model(
    ...
    train_loader = torch.utils.data.DataLoader(
        Dataset(root="/home/ubuntu/VLR/HW2/generative-modeling-master/datasets/CUB_200_2011_32", transform=ds_transforms),
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                # discriminator on it.
                ###################################################################
                # interp = interpolate_latent_space(gen, '/home/ubuntu/VLR/HW2/generative-modeling-master/gan/data_gan/Generations_Interpolate.png')
                rand_a = torch.rand(size=(1,), device='cuda')
                interp = rand_a * gen_batch + (1 - rand_a) * train_batch
            ...
            if iters % log_period == 0 and iters != 0:
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=amp_enabled):
                        ...
                        ##################################################################
                        generated_samples = gen(100)*0.5 + 0.5
                        ##################################################################
                        #                          END OF YOUR CODE                      #
                        ##################################################################
                    save_image(
