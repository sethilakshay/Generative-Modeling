...
def ae_loss(model, x):
    ...
    ##################################################################
    model_out = model.decoder(model.encoder(x))
...
def vae_loss(model, x, beta = 1):
    ...
    model_out = model.decoder(mu + log_std.exp() * torch.randn(size=mu.shape, device = 'cuda' if torch.cuda.is_available() else 'cpu'))
    recon_loss = nn.MSELoss(reduction = 'sum')(model_out, x)/len(x)
    
    var = log_std.exp()*log_std.exp()
    kl_loss = 0.5 * torch.mean(torch.sum(var + mu**2 -(torch.log(var) + 1), dim=1))
