...
def compute_discriminator_loss(
    ...
    grad_interp = torch.autograd.grad(outputs=discrim_interp, inputs=interp, grad_outputs=torch.ones_like(discrim_interp), create_graph=True)[0]
    grap_penalty = torch.pow(torch.linalg.norm(grad_interp) - 1, 2)
    loss_pt2 = lamb * grap_penalty.mean()
    loss = loss_pt1 + loss_pt2
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
